...
item:
  ...
  REQUIRE(h.item.id.id == 2);
  auto* bar = h.item.value.get_if<Bar>();
  REQUIRE(bar != nullptr);
  REQUIRE(bar->s == "hello");
}
...
item:
  ...
  REQUIRE(h.item.id.id == 3);
  auto* value = h.item.value.get_if<int>();
...
TEST_CASE("yaml_runtime_type: round trip")
{
  auto   reg = make_registry();
  Holder h;
  h.item.id = ouly::type_id{1};
  h.item.value.emplace<Foo>(Foo{42});
  ...
  REQUIRE(out.item.id.id == 1);
  auto* foo = out.item.value.get_if<Foo>();
  REQUIRE(foo != nullptr);
  REQUIRE(foo->x == 42);
}
...
TEST_CASE("yaml_runtime_type: vector of mixed runtime types")
{
  ...
  ListHolder lh;
  {
    ouly::runtime_type a;
    a.id = ouly::type_id{1};
    a.value.emplace<Foo>(Foo{7});
    lh.items.push_back(std::move(a));
  }
  {
    ouly::runtime_type b;
    b.id = ouly::type_id{2};
    b.value.emplace<Bar>(Bar{"world"});
  ...
  REQUIRE(out.items[0].id.id == 1);
  REQUIRE(out.items[0].value.get_if<Foo>() != nullptr);
  REQUIRE(out.items[0].value.get_if<Foo>()->x == 7);
  REQUIRE(out.items[1].id.id == 2);
  REQUIRE(out.items[1].value.get_if<Bar>() != nullptr);
  REQUIRE(out.items[1].value.get_if<Bar>()->s == "world");
}
...
TEST_CASE("yaml_runtime_type: customizable type/value field names (write + round trip)")
{
  ...
  ouly::yml::from_string(out, str, reg);
  REQUIRE(out.item.id.id == 1);
  REQUIRE(out.item.value.get_if<Foo>() != nullptr);
  REQUIRE(out.item.value.get_if<Foo>()->x == 42);
}
