...
TEMPLATE_TEST_CASE("EulerAngles: Validate euler_angles_t<TestType>::canonize", "[euler_angles_t<TestType>::canonize]",
                   float, double)
{
  ...
  CHECK((acl::to_degrees(acl::get_y(angles)) == Catch::Approx(-180.0f) ||
         acl::to_degrees(acl::get_y(angles)) == Catch::Approx(180.0f)));
...
TEMPLATE_TEST_CASE("EulerAngles: Validate euler_angles_t<TestType>::from_quat", "[euler_angles_t<TestType>::from_quat]",
                   float, double)
{
  acl::euler_angles_t<TestType> angles =
    acl::make_euler_angles(acl::make_quaternion(acl::vec3a_t<TestType>(0, 1, 0), acl::to_radians<TestType>(65.0f)));
  angles = acl::canonize(angles);
  CHECK(acl::to_degrees(angles.pitch) == Catch::Approx(0.0f));
  CHECK(acl::to_degrees(angles.yaw) == Catch::Approx(65.0f));
  CHECK(acl::to_degrees(angles.roll) == Catch::Approx(0.0f));
}

TEMPLATE_TEST_CASE("EulerAngles: Validate euler_angles_t<TestType>::from_quat_conjugate",
                   "[euler_angles_t<TestType>::from_quat_conjugate]", float, double)
{
  acl::euler_angles_t<TestType> angles = acl::make_euler_angles(
...
TEMPLATE_TEST_CASE("EulerAngles: Validate euler_angles_t<TestType>::from_mat4", "[euler_angles_t<TestType>::from_mat4]",
                   float, double)
{
...
TEMPLATE_TEST_CASE("EulerAngles: Validate euler_angles_t<TestType>::from_mat3", "[euler_angles_t<TestType>::from_mat3]",
                   float, double)
{
