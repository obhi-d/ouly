...
inline bool is_valid(aabb_t<scalar_t> const& box)
{
...
template <typename scalar_t>
inline vec3a_t<scalar_t> center(aabb_t<scalar_t> const& box)
{
  return half(add(box.r[1], box.r[0]));
}

template <typename scalar_t>
inline vec3a_t<scalar_t> size(aabb_t<scalar_t> const& box)
{
  return sub(box.r[1], box.r[0]);
}

template <typename scalar_t>
inline vec3a_t<scalar_t> half_size(aabb_t<scalar_t> const& box)
{
  return half(sub(box.r[1], box.r[0]));
}

template <typename scalar_t>
inline vec3a_t<scalar_t> corner(aabb_t<scalar_t> const& box, unsigned int i)
{
  return set(x(box.r[((i >> 2) & 1)]), y(box.r[((i >> 1) & 1)]), z(box.r[((i >> 0) & 1)]));
}

template <typename scalar_t>
inline aabb_t<scalar_t> append(aabb_t<scalar_t> const& b, vec3a_t<scalar_t> const& point)
{
  if constexpr (has_sse && std::is_same_v<scalar_t, float>)
...
template <typename scalar_t>
inline aabb_t<scalar_t> append(aabb_t<scalar_t> const& box, aabb_t<scalar_t> const& other)
{
  if constexpr (has_sse && std::is_same_v<scalar_t, float>)
...
template <typename scalar_t>
inline aabb_t<scalar_t> make_aabb_from_center_extends(vec3a_t<scalar_t> const& center, vec3a_t<scalar_t> const& extends)
{
  return aabb_t<scalar_t>{sub(center, extends), add(center, extends)};
}

template <typename scalar_t>
inline aabb_t<scalar_t> make_aabb_from_min_max(vec3a_t<scalar_t> const& i_min, vec3a_t<scalar_t> const& i_max)
{
