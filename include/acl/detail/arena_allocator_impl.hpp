...
template <typename ustrategy, typename umanager, typename usize_type, bool b_compute_stats>
struct arena_traits_t
{
  using size_type                       = usize_type;
  using strategy                        = ustrategy;
  using manager                         = umanager;
  using extension                       = typename strategy::extension;
  static constexpr bool k_compute_stats = b_compute_stats;
};

template <typename traits>
class arena_allocator_uimpl : public detail::statistics<typename traits::strategy, traits::k_compute_stats,
...
protected:
  ...
  using strategy  = typename traits::strategy;
  ...
  using statistics     = detail::statistics<strategy, traits::k_compute_stats, defrag_stats<traits::k_compute_stats>>;
...
public:
  ...
  void deallocate(ihandle node)
  {
    ...
    case merge_type::e_left_and_right:
      ...
      auto right_size = ibank.bank.blocks[block_link(right)].size;
      ibank.strat.erase(ibank.bank.blocks, right);
      ibank.strat.grow_free_node(ibank.bank.blocks, left, 
  ...
  void validate_integrity() const
  {
         ...
         arena_it != arena_end_it; ++arena_it)
    {
      ...
      for (auto blk_it = arena.block_order.begin(ibank.bank.blocks), blk_end_it = arena.block_order.end(ibank.bank.blocks);
...
template <typename ustrategy, typename umanager, typename usize_type, bool b_compute_stats>
class arena_allocator_impl
    : public arena_allocator_uimpl<arena_traits_t<ustrategy, umanager, usize_type, b_compute_stats>>
{
protected:
  using super = arena_allocator_uimpl<arena_traits_t<ustrategy, umanager, usize_type, b_compute_stats>>;
